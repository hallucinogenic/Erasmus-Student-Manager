    ...
    if [ -f "$file_uni" ]
        then
            ...
            do
                ...
                cp -R "universidades.ini" "backup/$date/"

                # Comprime-se a diretoria para um ficheiro tar.bz2
                tar -cjf "$date.tar.bz2" "$dir_sigla"
            ...
            echo "$date.tar.bz2" >> "backup/list_backups.ini"
    else
...
Backup_Remove()
{
    ...
    if [ -f "$file_backups" ]
        ...
        if [ $option -eq 0 ]
            ...
            filename=$(head -$option "backup/list_backups.ini" | tail -1)
